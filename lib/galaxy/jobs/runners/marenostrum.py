...
class MarenostrumJobRunner(AsynchronousJobRunner):
    ...
    def queue_job(self, job_wrapper):
        ...
        self._prepare_preferences()
        self._prepare_keys(job_wrapper.app.user_manager.by_id(job_wrapper.user_id).username, self.host_MN4, self.user_MN4)
        self._prepare_task(self.host_MN4, self.user_MN4)
        self.job_outputs = job_wrapper.compute_outputs()
        print("------------------TOOL--------------")
        print(job_wrapper.tool_working_directory)
        print(job_wrapper.dependencies)
        print(job_wrapper.tool.tool_requirements)
        print(job_wrapper.tool.command)
        print(job_wrapper.tool.tool_type)
        print(job_wrapper.tool.tool_shed)
        print(dir(job_wrapper.tool))
        print(job_wrapper.get_input_paths()[0].real_path)

        print(job_wrapper.tool.input_params)
        print(job_wrapper.tool.outputs)
        print(job_wrapper.tool.tool_dir)
        print(job_wrapper.tool_provided_job_metadata)
        print(job_wrapper.output_paths)

        print("------------------TOOL--------------")
        re_galaxy_wd = self.job_wrapper.working_directory.rsplit("/",2)[0]
        re_mn4_jobs_wd = self.jobs_MN4

        embed_metadata = asbool(self.job_wrapper.job_destination.params.get("embed_metadata_in_job", DEFAULT_EMBED_METADATA_IN_JOB))
        if not self.prepare_job(self.job_wrapper, include_metadata=embed_metadata):
            return

        with open(os.path.join(self.job_wrapper.working_directory,"tool_script.sh"), "r" ) as f:
            self.script = f.read()
        # ENVIAR ENTORNO CONDA A MN4
        print(self.app.config.get('tool_path') in job_wrapper.tool.tool_dir)
        print(self.app.config.get('tool_path'), job_wrapper.tool.tool_dir)

        if self.app.config.get('tool_path') in job_wrapper.tool.tool_dir:
            tool_full_path = [x for x in self.script.split("'") if job_wrapper.tool.tool_dir in x][0]
            self._send_default_tool(tool_full_path)   
        try:
            local_conda_path = [x for x in self.script.split("'")  if "_conda/envs/_" in x][0]
            self._send_conda_env(local_conda_path, self.conda_MN4)
        except:
            print("No conda env found")
        # ENVIAR ENTORNO DE CONDA BASE
        self._send_conda_env(self.app.config.get('tool_dependency_dir') + "/_conda/", self.conda_MN4)

        # ENVIAR DATASETS DE INPUT a MN4
        for path in job_wrapper.get_input_paths():
            self.send_input_to_mn4(path.real_path)
        # ENVIAR TOOLS LOCALES A MN4
        # REEMPLAZAR LOS SIGUIENTES PATHS EN EL SCRIPT QUE SE EJECUTARÁ
        # PATH DE DEPENDENCIA CONDA
        self.script = re.sub(self.app.config.get('tool_dependency_dir') + "/_conda/envs", self.conda_MN4, self.script) 
#        self.fix_conda_sh_paths()
        self.script = "source " + self.conda_MN4 + "/etc/profile.d/conda.sh\n" + self.script
        print (self.script)
        print("----------------")
        print(self.app.config.get('tool_dependency_dir') + "/_conda/", self.conda_MN4)
        print("----------------")
        # REPLACE LOCAL GALAXY CONDA PATH WITH MN4 CONDA PATH
        self.script = re.sub(self.app.config.get('tool_dependency_dir') + "/_conda", self.conda_MN4, self.script) 
        # REPLACE GALAXY TOOLS PATH WITH MN4 TOOLS PATH
        self.script = re.sub(self.app.config.get('tool_path'), self.tool_path, self.script) 
        # REPLACE LOCAL GALAXY DATASETS PATH WITH MN4 DATASETS PATH
        self.script = re.sub(self.app.config.get('file_path'), self.file_path, self.script)
        

        galaxy_id_tag = self.job_wrapper.get_id_tag()
        #try:
        #    self.write_executable_script(ajs.job_file, self.script)
        #except Exception:
        #    log.exception(f"({galaxy_id_tag}) failure writing job script", galaxy_id_tag)
        ...
        self._save_task()
        job_id = self.task.submit(queue_settings="debug", job_name=self._job_name(),
                                  local_run_script=self.script)
        
        job_wrapper.set_external_id(job_id)
        self.ajs = AsynchronousJobState(files_dir=self.job_wrapper.working_directory, job_id=job_id, job_wrapper=self.job_wrapper, job_destination=self.job_wrapper.job_destination)
        #self.ajs.job_id = job_id
    ...
    def recover(self, job, job_wrapper):
        ...
        self._prepare_preferences()
        self._prepare_keys(job_wrapper.app.user_manager.by_id(job_wrapper.user_id).username, self.host_MN4, self.user_MN4)
        self._recover_task(self.host_MN4, self.user_MN4, str(job_wrapper.job_id))
        ...
        log.info("Job being queued in Marenostrum")

    def _prepare_preferences(self):
        log.info("Getting user preferences")
        preferences = self.job_wrapper.app.user_manager.preferences(self.job_wrapper.app.user_manager.by_id(self.job_wrapper.user_id))
    ...
    def _prepare_keys(self, username, host, MN4_username):
        ...
        ssh_folder = os.path.join(self.app.config.get('mn4_ssh_keys_path'), username)
    ...
    def _save_task():
        ...
        self.task.save(self.task_cache + "/" + job_id + ".task")

    def _prepare_task(self, host, userid):
        self.task = slurm.Slurm(host, userid, False)
        self.task.set_credentials(self.credentials)

        #self._send_conda_env()
        #Send galaxy's working dir to Marenostrum designated folder
        id_path ="/".join(self.job_wrapper.working_directory.split("/")[-2:])
        self.MN4_wd = self.jobs_MN4 + "/" + id_path
        self.task.set_local_data_bundle(local_data_path=self.job_wrapper.working_directory)
    ...
    def _job_name(self):
        external_runjob_script = self.job_wrapper.get_destination_configuration("drmaa_external_runjob_script", None)
        galaxy_id_tag = self.job_wrapper.get_id_tag()

        # define job attributes
        self.job_name = 'g%s' % galaxy_id_tag
        if self.job_wrapper.tool.old_id:
            self.job_name += '_%s' % self.job_wrapper.tool.old_id
        if not self.redact_email_in_job_name and external_runjob_script is None:
            self.job_name += '_%s' % self.job_wrapper.user
    ...
    def _send_conda_env(self, conda_env_path, remote_env_path):
        #temp_conda_pack_dir = tempfile.mkdtemp()
        #conda_pack_file = temp_conda_pack_dir + "/" + os.path.basename(conda_env_path + ".tar.gz")
        #try:
        ...
        if self.local_checksum(conda_env_path + ".tar.gz") != self.remote_checksum(remote_env_path + ".tar.gz"):
            ...
            print("-----------------SENDING_CONDA_ENV_PACKED----------------")
            print(remote_env_path)
            self.task.send_input_data(remote_env_path, overwrite=False)
            print ("____________________CONDA_ENV_SENT_________")
        ...
        #shutil.rmtree(temp_conda_pack_dir)


    def create_log_files(self, output_file, exit_code_file, error_file):
        print("CREATING LOG FILES")
        self.task.get_output_data(self.job_wrapper.working_directory)
        output_files = self.task.task_data['output_data_bundle'].files
        print(output_files)
        remote_output = self.job_wrapper.working_directory + '/' + [out for out in output_files if "out" in out][0] 
        remote_error =  self.job_wrapper.working_directory + '/' + [err for err in output_files if "err" in err][0] 
        os.rename(remote_error, error_file)
        os.rename(remote_output, output_file)

        print(remote_output)
        print(output_file, exit_code_file,error_file)
    ...
    def remote_checksum(self, path):
        ...
        return md5sum
    def send_input_to_mn4(self, input_path):
        print(type(self.app.config.get('file_path')))
        print(type(input_path))
        basepath = re.sub(self.app.config.get('file_path'), "", input_path)
        print(self.task.ssh_session.run_command("mkdir -p " + self.file_path + os.path.dirname(basepath)))
        self.task.set_local_file(local_data_filepath=input_path)
        print(self.file_path + "/" + os.path.basename(input_path))
        self.task.send_input_data(self.file_path + "/" + os.path.dirname(basepath))

    def _send_default_tool(self, tool_path):
        print(tool_path) 
        self.task.set_local_file(local_data_filepath=tool_path)
        self.task.send_input_data(remote_base_path=self.tool_path + "/" + os.path.basename(self.job_wrapper.tool.tool_dir)) 
    ...
    def _get_outputs(self):
        for output in self.job_outputs:
            ...
            print(remote_path)
            print(self.task.ssh_session.run_sftp('get', remote_path, output_path))
